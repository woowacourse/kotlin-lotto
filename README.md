...
### 3차 피드백 정리
- Lotto의 toString()을 오버라이딩을 구조적으로 변환
- 객체간의 어색한 의존성을 줄이고 각 기능 역할 분리
- compareTo operator 학습 후 적용
- java integer class를 통해서 숫자 재활용(중복된 번호의 객체를 생성 할 수 없도록)
- 책임과 역할에 맞는 클래스명 및 함수명으로 수정
- 함수 분리를 통해서 함수 재활용
- 테스트의 기능 분리 및 목적을 상기시키며 분리