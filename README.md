...
### 자동 로또 발매기
- [x] 로또 지불 정보를 읽어 자동 로또 수량만큼 로또 리스트로 반환한다.

### 수동 로또 발매기
- [x] 로또 지불 정보와 구매할 번호들을 읽어 수동 로또 수량만큼 로또 리스트로 반환한다.
...
- [x] 로또 객체를 입력받아 자신이 가지고 있는 당첨 정보를 이용해 RANK를 반환한다.

### 등수
- [x] 일치 개수, 보녀스 일치여부를 입력받아 해당하는 RANK를 반환한다.
...
- [x] Lotto 인스턴스를 주 생성자에서 수동으로 생성할 수 있도록 한다.


## 동작 과정
1. 컨트롤러가 게임 실행 및 진행을 관리하며 비논리 오류는 null을 반환하며 종료한다.
2. 구매 금액과 수동 구매 수량을 입력 뷰에서 받아 `로또 구매 정보` 인스턴스를 생성한다.
3. `로또 구매 정보`를 입력 뷰에게 넘겨 수동 구매 수량만큼 수동 번호들을 가져온다
4. `로또 발매기`들로 수동 번호로 구매한 로또와 자동으로 구매 로또 리스트을 받아 `로또 묶음` 객체를 생성한다.
