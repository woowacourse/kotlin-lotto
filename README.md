...
- [x] 당첨 판별 로직을 LottoTicket에서 다른 클래스로 이동
- [x] GainLoss에서 if문으로 리턴하는 대신 when 적용 연습

- [x] 객체의 상태와 행위를 직접 나타내는 테스트명으로 변경
- [x] "로또 수량" 객체는 1 이상의 값만 가지도록 수정
- [x] 0을 허용하는 "수동 로또 수량" 및 "자동 로또 수량" 객체 추가
- [x] 패키지를 역할에 따라 구조화
- [ ] 로또 구입 금액의 테스트코드에서 구입 로또 수량 관련 변경된 요구사항 반영
- [ ] 접근 제어자 신경써서 수정