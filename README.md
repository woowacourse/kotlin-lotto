...
- [x] 수동으로 구매할 로또 수 입력
    ...
    - [x] 사용자가 잘못된 구입금액을 입력할 경우 IllegalArgumentException를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
        - [x] 수동으로 구매할 로또 수가 1 이상의 숫자가 아닌 경우 예외 처리한다.


- [ ] 수동으로 구매할 번호 입력
  - [ ] "수동으로 구매할 번호를 입력해 주세요."라는 입력 안내 문구를 출력한다.
  - [ ] 사용자가 잘못된 구매 번호를 입력할 경우 IllegalArgumentException를 발생시키고, "[ERROR]"로 시작하는 에러 메시지를 출력 후 그 부분부터 입력을 다시 받는다.
    - [ ] 구매 번호는 쉼표(,)를 기준으로 구분한다.
    - [ ] 구매 번호의 개수가 6개가 아닌 경우 예외 처리한다.
    - [ ] 구매 번호가 1~45사이의 숫자가 아닌 경우 예외 처리한다.
    - [ ] 구매 숫자가 중복되는 경우 예외 처리한다.
