...
class LotteriesTest {
    ...
    @Test
    fun `로또 뭉치들을 합쳐서 하나의 로또 뭉치로 만들 수 있다_리팩터링 이전`() {
        val lotteries =
            Lotteries(
                listOf(
    ...
    @Test
    fun `로또들에 대해 당첨 로또(당첨 번호 & 보너스 번호)를 비교하여 최종 당첨 결과를 구한다_리팩터링 이전`() {
        val lotteries =
            Lotteries(
                listOf(
                    Lottery.of(1, 2, 3, 4, 5, 6),
    ...
    fun `로또 여러 장을 저장할 수 있다`() {
        val lotteries =
            Lotteries(
                listOf(
                    Lottery.of(ExplicitNumbersGenerator(listOf(1, 2, 3, 4, 5, 6))),
                    Lottery.of(ExplicitNumbersGenerator(listOf(1, 2, 3, 4, 5, 6))),
                ),
            )
        assertThat(lotteries.lotteries.size).isEqualTo(2)
    ...
    fun `로또 뭉치들을 합쳐서 하나의 로또 뭉치로 만들 수 있다`() {
        val lotteries =
            Lotteries(
                listOf(
                    Lottery.of(ExplicitNumbersGenerator(listOf(1, 3, 5, 7, 9, 11))),
                    Lottery.of(ExplicitNumbersGenerator(listOf(2, 4, 6, 8, 10, 12))),
                ),
            )
        val otherLotteries =
            Lotteries(
                listOf(
                    Lottery.of(ExplicitNumbersGenerator(listOf(2, 4, 6, 8, 10, 12))),
                    Lottery.of(ExplicitNumbersGenerator(listOf(3, 6, 9, 12, 15, 18))),
                ),
            )
        assertThat(lotteries + otherLotteries).isEqualTo(
            Lotteries(
    ...
    fun `로또들에 대해 당첨 로또(당첨 번호 & 보너스 번호)를 비교하여 최종 당첨 결과를 구한다`() {
        val lotteries =
            Lotteries(
                listOf(
                    Lottery.of(ExplicitNumbersGenerator(listOf(1, 2, 3, 4, 5, 6))),
                    Lottery.of(ExplicitNumbersGenerator(listOf(4, 5, 6, 7, 8, 9))),
                    Lottery.of(ExplicitNumbersGenerator(listOf(4, 5, 6, 10, 11, 12))),
                ),
            )
        val winningLottery =
            WinningLottery(
