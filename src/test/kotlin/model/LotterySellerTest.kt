...
class LotterySellerTest {
    ...
    fun `1000원 미만의 거스름돈이 있다면 돌려주어야 한다`() {
        val lotterySeller = LotterySeller(Money(BigDecimal(5800)))
        assertThat(lotterySeller.exchange())
            .isEqualTo(Money(BigDecimal(800)))
    }

    @Test
    fun `로또 구매 수량을 알려준다`() {
        val lotterySeller = LotterySeller(Money(BigDecimal(3800)))
    ...
    fun `천원 이하의 금액은 예외를 던진다`() {
        assertThrows<IllegalArgumentException> {
            LotterySeller(Money(BigDecimal(500)))
        }
    }
    ...
    fun `100,000원 초과 구매시 예외를 던진다`() {
        assertThrows<IllegalArgumentException> {
            LotterySeller(Money(BigDecimal(500_000)))
        }
    }
    ...
    fun `1,000원 이상 100,000이하의 금액에 대해서는 정상적으로 동작한다`(amount: Int) {
        assertDoesNotThrow {
            LotterySeller(Money(BigDecimal(amount)))
        }
    }
}
