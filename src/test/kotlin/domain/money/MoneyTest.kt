...
class MoneyTest {
    @Test
    fun `0이상의 숫자 값이 주어졌을 때, 오류가 발생하지 않는다`() {
        assertDoesNotThrow {
            Money(10000)
        }
    }
    ...
    @Test
    fun `0미만의 값이 주어졌을 때, IllegalArgumentException이 발생한다`() {
    ...
    fun `정수가 주어졌을 때, 1000으로 나눈 몫을 반환한다`(money: Int, expected: Int) {
    ...
    @Test
    fun `1000원 짜리 수동 로또의 개수가 주어졌을 때, 남은 금액을 반환한다`() {
    ...
    @Test
    fun `구매금액보다 사려는 로또 금액이 더 많을 때, IllegalStateException이 발생한다`() {
