...
class LottoStatisticsTest {
    ...
    companion object {
        ...
        fun provideLottoAndBonusMatchResult(): Stream<Arguments> {
            return Stream.of(
                Arguments.arguments(Lotto(listOf(1, 2, 3, 4, 5, 6).map { number -> LottoNumber(number) }.toSet()), false),
        ...
        fun provideLottoAndRankMatchResult(): Stream<Arguments> {
            return Stream.of(
                Arguments.arguments(Lotto(listOf(1, 2, 3, 4, 5, 6).map { number -> LottoNumber(number) }.toSet()), Rank.FIRST),
                Arguments.arguments(Lotto(listOf(1, 2, 3, 4, 5, 7).map { number -> LottoNumber(number) }.toSet()), Rank.SECOND),
                Arguments.arguments(Lotto(listOf(1, 2, 3, 4, 5, 10).map { number -> LottoNumber(number) }.toSet()), Rank.THIRD),
                Arguments.arguments(Lotto(listOf(1, 2, 3, 4, 10, 11).map { number -> LottoNumber(number) }.toSet()), Rank.FOURTH),
                Arguments.arguments(Lotto(listOf(1, 2, 3, 11, 12, 45).map { number -> LottoNumber(number) }.toSet()), Rank.FIFTH),
                Arguments.arguments(Lotto(listOf(1, 2, 11, 12, 15, 45).map { number -> LottoNumber(number) }.toSet()), Rank.MISS),
            )
        }
    }
}
