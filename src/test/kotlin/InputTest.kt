...
class InputTest {
    ...
    fun `올바른 보너스 번호 입력`(bonusNumber: String) {
        val winning = Winning(listOf(1, 2, 3, 4, 5, 6))
        assertDoesNotThrow { InputValidator.validateBonusNumber(bonusNumber, winning) }
    }
    ...
    fun `보너스 번호 입력 예외 처리 (보너스 번호가 1~45사이의 숫자가 아닌 경우)`(bonusNumber: String) {
        val winning = Winning(listOf(1, 2, 3, 4, 5, 6))
        assertThrows<IllegalArgumentException> { InputValidator.validateBonusNumber(bonusNumber, winning) }
    }
