...
class ValidatorTest {
    ...
    fun `수동 로또의 개수는 0개 이상이어야 한다`(manualAmount: Int) {
        val amount = 10
        assertDoesNotThrow { Validator.manualAmountValidator(manualAmount, amount) }
    }
    ...
    fun `수동으로 구매할 로또의 수는 정수로 입력받아야 한다`(manualAmount: Int) {
        val amount = 10
        assertThrows<IllegalArgumentException> { Validator.manualAmountValidator(manualAmount, amount) }
    }
