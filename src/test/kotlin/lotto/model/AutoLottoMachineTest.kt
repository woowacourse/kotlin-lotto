...
class AutoLottoMachineTest {
    ...
    fun `사전에 정의된 범위로 로또를 생성한다`() {
        val autoBuyedLottos = AutoLottoMachine.createLottos(LottoBuyBudget(Lotto.PRICE_PER_LOTTO), fixedLottoNumbersGenerator).first()
    ...
    fun `로또 한장의 가격이 5,000원일 때 2만원을 투입하면 총 4장의 로또를 발급한다`() {
        ...
        assertThat(autoBuyedLottos.size).isEqualTo(4)
    }
