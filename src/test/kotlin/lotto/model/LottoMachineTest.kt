...
class LottoMachineTest {
    ...
    fun `사전에 정의된 범위로 로또를 생성한다`() {
        val buyedLotto = LottoMachine.createLottos(LottoBuyPrice(Lotto.LOTTO_PRICE), fixedLottoNumbersGenerator).first()
    ...
    fun `로또 한장의 가격이 5,000원일 때 2만원을 투입하면 총 4장의 로또를 발급한다`() {
        val buyedLottos = LottoMachine.createLottos(LottoBuyPrice(20000, 5000), fixedLottoNumbersGenerator)
        assertThat(buyedLottos.size).isEqualTo(4)
    }

    @Test
    fun `로또 한장의 가격이 1,000원일 때 1만원을 투입하면 총 10장의 로또를 발급한다`() {
        val buyedLottos = LottoMachine.createLottos(LottoBuyPrice(10000, 1000), fixedLottoNumbersGenerator)
