...
internal class LottoTicketTest {
    ...
    internal fun `로또 티켓에 중복된 숫자가 있는 경우`() {
        val lottoNumbers: MutableList<LottoNumber> = (1..5).map { LottoNumber.from(it) }.toMutableList()
    ...
    internal fun `로또 숫자 정렬`() {
        val lottoNumbers: MutableList<LottoNumber> = (2..6).map { LottoNumber.from(it) }.toMutableList()
        lottoNumbers.add(LottoNumber.from(1))
        val lottoTicket = LottoTicket.from(lottoNumbers)
    ...
    internal fun `당첨 번호와 겹치는 번호 개수`() {
        val lottoNumbers: List<LottoNumber> = (1..6).map { LottoNumber.from(it) }
        val winningNumbers: List<LottoNumber> = (2..7).map { LottoNumber.from(it) }
