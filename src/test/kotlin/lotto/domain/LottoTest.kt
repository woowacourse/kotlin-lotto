...
class LottoTest {
    ...
    fun `로또는 중복된 숫자를 가지면 오류가 발생한다`() {
        assertThrows<IllegalArgumentException> {
            Lotto(listOf(LottoNumber(1), LottoNumber(1), LottoNumber(3), LottoNumber(4), LottoNumber(5), LottoNumber(6)))
        }
    }

    @Test
    fun `로또 번호는 오름차순으로 정렬된다`() {
        val lotto = Lotto(listOf(LottoNumber(7), LottoNumber(2), LottoNumber(3), LottoNumber(4), LottoNumber(5), LottoNumber(6)))
        assertThat(
            lotto.numbers,
