...
class LottoResultTest {
    @Test
    fun `6개 일치 시 1등`() {
        val wonLotto = WinningLotto(Lotto((1..6).map { LottoNumber(it) }.toSet()), bonusNumber = LottoNumber(7))
    ...
    @Test
    fun `5개 일치 시 3등`() {
        val wonLotto = WinningLotto(Lotto((1..6).map { LottoNumber(it) }.toSet()), LottoNumber(7))
